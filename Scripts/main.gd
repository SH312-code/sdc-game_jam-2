...
func _process(delta: float) -> void:
	dt += delta
	if dt > 1 and game and existing_enemies < max_enemy: # This is to prevent 30 enemys on the field in the begining, diffictlty climbs with experince and also bricks count as childeren
		dt -= 1
		existing_enemies += 1
		#spawn new enemy
...
func update_score(): # https://docs.godotengine.org/en/stable/getting_started/first_2d_game/06.heads_up_display.html
	score += 1
	existing_enemies -= 1
	var random_progress = randi_range(1,4)
